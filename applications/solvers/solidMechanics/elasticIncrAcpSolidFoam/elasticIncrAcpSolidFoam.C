...
#                 include "aitkenRelaxation.H"
		...
		{	  
	      ...
	      gradDU = fvc::grad(DU); // use leastSquaresSolidInterface grad scheme
...
#         include "updateCrack.H"
	...
	{
	  if ( returnReduce(cohesivePatchDUFixedModePtr->size(), sumOp<label>()) )
	    {
	      Pout << "Number of faces in crack: " << cohesivePatchDUFixedModePtr->size()
	       	   << endl;
