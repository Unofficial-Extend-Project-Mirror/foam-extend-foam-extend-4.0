    ...
    if (Pstream::parRun())
    {
        ...
        forAll (faceNormals, facei)
        {
            ...
            else if (mag(magSf - nbrMagSf)/avSf > polyPatch::matchTol_)
            {
                ...
                const labelList& ptLabels = mesh.faces()[start()+facei];

                bool glPoint = false;
                forAll(ptLabels, ptI)
                {
                    if (findIndex(spLabels, ptLabels[ptI]) != -1)
                    {
                        glPoint = true;
                    }
                }

                Pout << "Global point face: " << glPoint << endl;

                faceList f(1, mesh.faces()[start()+facei]);

                PrimitivePatch<face, List, pointField, point> patch
                (
                    f,
                    mesh.points()
                );
                ...
                FileName() << "face_" << Pstream::myProcNo() << ".vtk";
                patch.writeVTK(FileName.str());
