...
//                     << abort(FatalError);
        ...
        {
            // Grab RBF motion solver
//             const motionSolver& ms =
//                 fluidMesh().objectRegistry::lookupObject<motionSolver>
//                 (
//                     "dynamicMeshDict"
//                 );

//             const dynamicMotionSolverFvMesh& dmsMesh =
//                 refCast<const dynamicMotionSolverFvMesh>(fluidMesh_);
            ...
            if (false)
//             if (isA<totalVelocityLaplacianFvMotionSolver>(ms))
            {
                ...
                Pout << refAllPoints.size() << endl;
                Pout << fluidMesh_.time().constant() << endl;
                Pout << polyMesh::meshSubDir << endl;

                {
                    ...
                    forAll(mp, pI)
                    {
//                         p[pI] = oldAllPoints[mp[pI]];
                        p[pI] = refAllPoints[mp[pI]];
                    }
                    ...
                    Info << "_" << average(C) << endl;
                }

                vectorField totalMotionUFluidPatch =
                    fluidPatchPointsDispl
                  - fluidPatchPointsDisplPrev;

//                 forAll (totalMotionUFluidPatch, pointI)
//                 {
//                     totalMotionUFluidPatch[pointI] +=
//                     (
...
//                 }
                ...
                totalMotionUFluidPatch /= flow().runTime().deltaT().value();

                motionUFluidPatch == totalMotionUFluidPatch;
            }
            else
            {
                motionUFluidPatch ==
