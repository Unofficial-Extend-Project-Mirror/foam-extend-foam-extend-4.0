...
//     Pout << "addedPoints.size(): " << addedPoints.size() << endl;
    ...
    // ZT, 22-May-2014
    SLList<labelPair> procPatchEdgeSet;
    SLList<label> sharedEdgeSet;
    labelList oldCrackTipEdges;

    const labelList& glEdges =
        mesh.globalData().sharedEdgeLabels();
    const labelList& glEdgeAddr =
    ...
    )
    {
        ...
        forAll (curFaces, faceI)
        {
            if (!mesh.isInternalFace(curFaces[faceI]))
            {
                ...
                {
                    label glEdge =
                        (findIndex(glEdges, meshEdges[curEdgeID]) != -1);

                    if (glEdge)
                    {
                        sharedEdgeSet.insert(meshEdges[curEdgeID]);
                    }
                    else
                    {
                        labelPair patchAndEdge
                        (
                            patchID,
                            findIndex
                            (
                                mesh.boundaryMesh()[patchID].meshEdges(),
                                meshEdges[curEdgeID]
                            )
                        );

                        procPatchEdgeSet.insert(patchAndEdge);
                    }

                    // ZT: Will be checked later
                    edgeIsInternal = false;
                }
...
//                 break;
    ...
    // ZT, 29-May-2014
    // Re-check edges at processor boundaries by checking
    // state at the neighbour processor
#   include "checkProcEdgesAtNeighbourProcessor.H"
...
//     Pout << "nAddedPoints: " << nAddedPoints << endl;
    ...
    // Create the map of faces in the master cell layer
    // (neighbour cells for no flip)
    const labelList& mc =
        mesh.faceZones()[crackZoneID_.index()].masterCells();

//     if (mesh.faceZones()[crackZoneID_.index()].size())
//     {
//         Pout << "n master face "
//             << n[mesh.faceZones()[crackZoneID_.index()][0]] << endl;
//     }
    ...
    forAll (coupledFacesToBreak_, faceI)
    {
        ...
        // Look for old crack tips
#       include "lookForOldCrackTips.H"
    }

    reduce(checkSharedEdge, maxOp<labelList>());

//     Pout << "checkSharedEdge: " << checkSharedEdge << endl;

#   include "checkSharedEdgesForCrackTip.H"

    forAll (coupledFacesToBreak_, faceI)
    {
        const label& curFaceID = coupledFacesToBreak_[faceI];

        if (oldCrackTipEdges.size() == 0)
        {
            ref.setAction
            (
                polyModifyFace
...
#   include "detachOldCrackTipAtProcessorBoundary.H"
