...
Foam::tmp<Foam::volScalarField> Foam::multiMaterial::Ep
(
    const volScalarField& epsEq
    ) const
{
    tmp<volScalarField> tresult
    (
...
Foam::scalar Foam::multiMaterial::sigmaY
(
...
Foam::scalar Foam::multiMaterial::dSigmaY
(
    const scalar epsilonPEq,
    const label cellID
) const
{
    ...
    {
        result +=
            indicator(lawI, cellID)*laws[lawI].dSigmaY(epsilonPEq, cellID);
    }
...
{
    // Plasticity is needed if any of the materials need it
    const PtrList<rheologyLaw>& laws = *this;
...
bool Foam::multiMaterial::nonLinearPlasticity() const
{
    const PtrList<rheologyLaw>& laws = *this;

    forAll(laws, lawI)
    {
        if (laws[lawI].nonLinearPlasticity())
        {
            return true;
        }
    }
