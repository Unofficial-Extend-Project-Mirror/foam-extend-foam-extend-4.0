...
void constitutiveModel::correctf()
{
    if (plasticityStressReturnPtr_.valid())
    {
        plasticityStressReturnPtr_->correctf();
    }
    else
    {
        //lawPtr_->correctf(); // no reason to define yet
//         lawPtr_->correct();
        rheologyLawPtr_->correct();
    }
}
...
const volSymmTensorField& constitutiveModel::epsilonP() const
{
  return plasticityStressReturnPtr_->epsilonP();
}
...
// Return first Lame's coefficient
tmp<volScalarField> constitutiveModel::mu
(
