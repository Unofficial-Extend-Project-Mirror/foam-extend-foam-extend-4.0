...
m4_define(nbrPassageUpstream, 4)
m4_define(nbrPassageDownstream, 4)
// Make sure we are properly aligned for sampling inside the mesh volume
m4_define(startAngleOffset, calc(360/nbrPassageUpstream/2))

// Angle span for section A_BB
m4_define(startAngleSectionA_BB, rad(calc(startAngleOffset +  0.0)))
m4_define(stopAngleSectionA_BB,  rad(calc(startAngleOffset + 360/nbrPassageUpstream)))
m4_define(angleSpanSectionA_BB,  calc(stopAngleSectionA_BB - startAngleSectionA_BB))

// Angle span for section B_F
m4_define(startAngleSectionB_F, rad(calc(startAngleOffset +  0.0)))
m4_define(stopAngleSectionB_F,  rad(calc(startAngleOffset + 360/nbrPassageDownstream)))
...
//Plane BB:     // This is where we put the ggi interface
m4_define(zBB, -0.10)
m4_define(rBB, rIn)
...
(
    ...
    //Plane B:
    arc B1 B2 (calc(rB*cos(startAngleOffset + (angleSpanSectionB_F)/2)) calc(rB*sin(startAngleOffset + (angleSpanSectionB_F)/2)) zB)

    //Plane C:
    arc C1 C2 (calc(rC*cos(startAngleOffset + (angleSpanSectionB_F)/2)) calc(rC*sin(startAngleOffset + (angleSpanSectionB_F)/2)) zC)

    //Plane D:
    arc D1 D2 (calc(rD*cos(startAngleOffset + (angleSpanSectionB_F)/2)) calc(rD*sin(startAngleOffset + (angleSpanSectionB_F)/2)) zD)

    //Plane E:
    arc E1 E2 (calc(rE*cos(startAngleOffset + (angleSpanSectionB_F)/2)) calc(rE*sin(startAngleOffset + (angleSpanSectionB_F)/2)) zE)

    //Plane F:
    arc F1 F2 (calc(rF*cos(startAngleOffset + (angleSpanSectionB_F)/2)) calc(rF*sin(startAngleOffset + (angleSpanSectionB_F)/2)) zF)
);
...
(
   ...
   (
    ...
    ggi B_UPSTREAM   // BB : master
    (
       (BB0 BB2 BB1 BB0)
    )

    ggi B_DOWNSTREAM  // B : master
    (
       (B0 B2 B1 B0)
    )
