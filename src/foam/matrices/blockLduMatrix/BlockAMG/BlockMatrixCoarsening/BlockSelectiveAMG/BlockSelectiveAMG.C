...
Foam::BlockSelectiveAMG<Type>::epsilon_
(
    "samgWeightFactor",
    0.25
);
...
template<class Type>
void Foam::BlockSelectiveAMG<Type>::calcCoarsening()
{
//------------------------------------------------------------------------------
//             MATRIX DATA: ADDRESSING and COEFF NORMS
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
    ...
    //HJ No need for tmp: you will not copy this one
...
//------------------------------------------------------------------------------
    ...
    // Mark equations
    labelList rowLabel(matrixSize, UNDECIDED);
    ...
    while (!equationWeight.empty())
    {
        ...
        if (rowLabel[topElement] == UNDECIDED)
        {
            rowLabel[topElement] = COARSE;
            nCoarseEqns_++;
            ...
            )
            {
                ...
                if (rowLabel[neighbour] == UNDECIDED)
                {
            ...
            )
            {
                ...
                if (rowLabel[neighbour] == UNDECIDED)
                {
                    ...
                    )
                    {
                        ...
                        if (rowLabel[acquaintance] == UNDECIDED)
                        {
...
//------------------------------------------------------------------------------
//                       CALCULTING PROLONGATION WEIGHTS
//------------------------------------------------------------------------------
...
#   endif
    ...
    if (nCoarseEqns_ > this->minCoarseEqns() && 3*nCoarseEqns_ <= 2*matrixSize)
    {
        coarsen_ = true;
    }
...
template<class Type>
Foam::BlockSelectiveAMG<Type>::BlockSelectiveAMG
(
...
Foam::BlockSelectiveAMG<Type>::restrictMatrix() const
{
    if (!coarsen_)
    {
...
//------------------------------------------------------------------------------
    ...
    // COARSE matrix. This number will be used for initialisation of arrays for
    // the constructor of the coarse matrix.
    ...
    for (label ir = 0; ir < nCoarseEqns_; ir++)
    {
        ...
        for (label indexR = rowStartR[ir]; indexR < rowStartR[ir + 1]; indexR++)
        {
            ...
            // LOWER TRIANGLE
            // Go into the corresponding row of A, to find if
            ...
            // DIAGONAL
            // Find off-diagonals in the COARSE matrix which come from the
            // diagonal of the FINE matrix A
            for
            (
...
//------------------------------------------------------------------------------
    ...
    TypeCoeffField coarseUpper(nCoarseCoeffs);
    TypeCoeffField coarseDiag(nCoarseEqns_);
    TypeCoeffField coarseLower(nCoarseCoeffs);

    // Reinitialise label vector
    coeffLabel = -1;
    nCoarseCoeffs = 0;
...
//------------------------------------------------------------------------------
    ...
    )
    {
        ...
        for (label ir = 0; ir < nCoarseEqns_; ir++)
        {
                ...
                )
                {
                    ...
                    )
                    {
                        ...
                        else
                        {
                            ...
                            label help = coeffLabel[jp];

                            // Save address if contribution to this coefficient
                            // appears for the first time
                            if (help == -1)
                            {
                                help = nCoarseCoeffs;
                                coeffLabel[jp] = nCoarseCoeffs;

                                // Grab addressing
                                coarseOwner[nCoarseCoeffs] = ir;
                                coarseNeighbour[nCoarseCoeffs] = jp;

                                nCoarseCoeffs++;
                            }
                            activeCoarseUpper[help] += ra*coeffP[indexP];
                        }
                    }
                }
                // FINE UPPER TRIANGLE
            ...
            {
                coeffLabel[coarseNeighbour[j]] = -1;
            }
        }
...
//------------------------------------------------------------------------------
        ...
        squareTypeField& activeCoarseMatrixUpper = coarseUpper.asSquare();
        squareTypeField& activeCoarseMatrixDiag = coarseDiag.asSquare();
        squareTypeField& activeCoarseMatrixLower = coarseLower.asSquare();


        // Insert coarse matrix diagonal
        for (label i = 0; i < nCoarseEqns_; i++)
        {
            activeCoarseMatrixDiag[i] = activeCoarseDiag[i];
        }

        // Insert coarse matrix upper and lower triangle
        for (label index = 0; index < nCoarseCoeffs; index++)
        {
            activeCoarseMatrixUpper[index] = activeCoarseUpper[index];
            activeCoarseMatrixLower[index] = activeCoarseLower[index];
        }
    ...
    return autoPtr<BlockAMGLevel<Type> >(NULL);
}


template<class Type>
void Foam::BlockSelectiveAMG<Type>::restrictResidual
(
...
template<class Type>
void Foam::BlockSelectiveAMG<Type>::prolongateCorrection
(
    Field<Type>& x,
