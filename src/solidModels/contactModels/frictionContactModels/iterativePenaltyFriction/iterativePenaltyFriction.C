...
namespace Foam
{
    ...
    addToRunTimeSelectionTable(frictionContactModel, iterativePenaltyFriction, dictionary);
}
	...
	if(magSlavePressure[faceI] > 1e5) //SMALL)
	  {
	    ...
	    // 	//slaveTrac += frictionPenaltyScale_*( (-slipTrac * (slip/mag(slip))) - slaveTrac );
	    ...
	    //- increment the slave shear traction
	    //- so we add an increment of shear force based on the shear displacement then
	    //- no more force is added when the shear displacement is zero and we limit the traction when
	    //- the shear force has reached frictionalCoefficient_*magSlavePressure[faceI]
	    //- if the displacement and traction are in the same direction then this will also
	    //- reduce the traction
   ...
   )
  {
	...
	if(mag(globalSlaveTraction[facei]) > SMALL)
	  ...
	  //numFaceFaces > 1) // don't smooth end/corner faces
	  {
	    ...
	    //   oscillationCorrFac_*globalSlaveTraction[facei] + (1.0-oscillationCorrFac_)*(avTracMag*dir);
	    globalSlaveTraction[facei] =
