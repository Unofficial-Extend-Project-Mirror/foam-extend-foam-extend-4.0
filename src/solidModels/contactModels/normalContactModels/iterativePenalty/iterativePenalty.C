  ...
  forAll(totalSlavePointTrac_, pointI)
    {
      ...
      if(slavePointPenetration_[pointI] < -contactGapTol_) //-I had this before < 0.0) 
	{
	  //contactStep = true;
      ...
      //- else if point is within contact tolerance then don't add any more traction
      ...
      else if((totalSlavePointTrac_[pointI] & slavePointNormals[pointI]) < 0.0)
	{
	  ...
	  totalSlavePointTrac_[pointI] +=
	    ( slavePointNormals[pointI] * returnPenaltyFac * slavePointPenetration_[pointI] );
