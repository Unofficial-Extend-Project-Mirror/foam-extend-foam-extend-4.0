...
void yamadaMises::correct()
{
  ...
  // Equivalent strain
    ...
    const volScalarField sigmaEq
      (
       "sigmaEq", 
       sqrt(1.5*magSqr(dev(newSigma))) + dimensionedScalar("SMALL", dimPressure, SMALL)
       );
    ...
    const volScalarField epsilonPEq = sqrt((2.0/3.0)*magSqr(dev(oldEpsilonP+DEpsilonP_)));
    const volScalarField oldEpsilonPEq("oldEpsilonPEq", sqrt((2.0/3.0)*magSqr(dev(oldEpsilonP))));
    ...
    forAll (betaI, cellI)
    {
        ...
        {
	  // philipc: this means that once beta=1 then it can never change back to zero
	  // as DEpsilonEq will probably always be finite
	  // I will add a check here if the cell has unloaded from the yield surface
	  //betaI[cellI] = 1.0;
  ...
  void yamadaMises::updateYieldStress()
  {
    ...
    //volScalarField oldSigmaEq("oldSigmaEq", sqrt(1.5*magSqr(dev(newSigma-DSigma))));
    //volScalarField Ep_ = constitutiveModel_.Ep(epsilonPEq);
    ...
    forAll(sigmaY_.boundaryField(), patchI)
        ...
        if (!sigmaY_.boundaryField()[patchI].coupled())
	  {
	    ...
	    const scalarField& epsilonPEqPatch = epsilonPEq_.boundaryField()[patchI];
		...
		if(betaPatch[faceI] > SMALL)
		  {
		    ...
		    sigmaYPatch[faceI] = constitutiveModel_.sigmaY(epsilonPEqPatch[faceI], faceCells[faceI]);
		  }
