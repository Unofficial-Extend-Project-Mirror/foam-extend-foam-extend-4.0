...
void aravasMises::correct()
{
  ...
  volSymmTensorField oldSigmaElastic = 2.0*mu*oldEpsilonElastic + lambda*tr(oldEpsilonElastic)*I;
  ...
  volSymmTensorField newSigmaElastic = 2.0*mu*newEpsilonElastic + lambda*tr(newEpsilonElastic)*I;
  ...
  forAll (DEpsilonP_, cellI)
	...
	{
	  plasticN_[cellI] = (3.0/(2.0*newSigmaEqElastic[cellI]))*newSigmaDevElastic[cellI];
	}

      // J2 yield function
      scalar fy = Foam::pow((newSigmaEqElastic[cellI]/sigmaY_[cellI]),2.0) - 1.0;
      ...
      else
	{
	  ...
	  aravasNewtonLoop(DEpsilonPEq_[cellI], s0, s00, G, ebart, qe, cellI, maxMagDEpsilon);
	  ...
	  forAll (plasticN_.boundaryField()[patchI], faceI)
	      ...
	      else
		{
		  plasticN_.boundaryField()[patchI][faceI] =
		    (3.0/(2.0*newSigmaEqElastic.boundaryField()[patchI][faceI]))*newSigmaDevElastic.boundaryField()[patchI][faceI];
		}
	      ...
	      else
		{
		  ...
		  // Calculate deq and s0
		  aravasNewtonLoop(DEpsilonPEq_.boundaryField()[patchI][faceI], s0, s00, G, ebart, qe, faceCells[faceI], maxMagDEpsilon);
  ...
  // and beta is used to partition the step into fully elastic and elastic-plastic
  ...
  scalar aravasMises::qfun (const scalar qe, const scalar deq, const scalar G) const
  {
    return (qe - 3*G*deq);
  }
  ...
  scalar aravasMises::s0fun (const scalar ebar, const label cellID) const
  {
    ...
    return constitutiveModel_.sigmaY(ebarNew, cellID); //strainYieldStressXY(ebarNew);
  }
   ...
   ) const
  {
	...
	if (i > MaxNewtonIter_-2)
	  {
	    Warning << "Aravas plasticity not converging, fx is " << mag(fdeq) <<endl;
	  }		
      }
    // while ( (mag(fdeq) > LoopTol_) && ++i < MaxNewtonIter_); 
    while ( (mag(residual) > LoopTol_) && ++i < MaxNewtonIter_); // use change in deq instead
