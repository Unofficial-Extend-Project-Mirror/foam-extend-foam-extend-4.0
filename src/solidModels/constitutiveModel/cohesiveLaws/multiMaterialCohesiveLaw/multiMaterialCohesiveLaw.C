...
namespace Foam
{
    ...
    addToRunTimeSelectionTable(cohesiveLaw, multiMaterialCohesiveLaw, dictionary);
}
...
Foam::scalar Foam::multiMaterialCohesiveLaw::indicator(const label index, const label cellID) const
{
...
Foam::scalar Foam::multiMaterialCohesiveLaw::interfaceID(const label mat1, const label mat2) const
{
  ...
  if(interfaceLawID == -1)
      ...
      forAll(interfaceCohesiveLaws_, lawI)
	{
      ...
      if(interfaceLawID == -1)
	{
	  FatalError << "Cannot find cohesive interfaceLaw "
...
:
    ...
    PtrList<cohesiveLaw>(),
    materials_(sigma.mesh().objectRegistry::lookupObject<volScalarField>("materials")),
    // (
    //  IOobject
    //  (
    //   "materials",
    ...
    //  )
{
    ...
    const constitutiveModel& conModel = mesh().objectRegistry::lookupObject<constitutiveModel>("rheologyProperties");
    if(conModel.law().type() == "multiMaterial")
      {
	const multiMaterial& mulMatLaw = refCast<const multiMaterial>(conModel.law());

	if(laws.size() != mulMatLaw.size())
	  {
	    FatalError << "There should be the same number of cohesive laws "
		       << "as rheology laws" << nl
		       << "Currently there are " << mulMatLaw.size() << " rheology laws "
    ...
    if( mag(interfaceLawEntries.size() - (laws.size()*(laws.size()-1)/2)) > SMALL )
      {
	// number of interfaces is a trianular number of number of materials
	// ((n)*(n-1)/2)
	FatalError << "There are " << interfaceLawEntries.size() << " interface cohesive"
		   << " laws defined, but there should be " << (laws.size()*(laws.size()-1)/2)
		   << "," << nl
...
Foam::tmp<Foam::volScalarField> Foam::multiMaterialCohesiveLaw::materials() const
{
    return materials_;
}

Foam::tmp<Foam::surfaceScalarField> Foam::multiMaterialCohesiveLaw::sigmaMax() const
{
    ...
    forAll(mesh().boundary(), patchI)
    {
	  ...
	  scalarField localPatchMaterials = materials_.boundaryField()[patchI].patchInternalField();
	  scalarField globalPatchMaterials = crackerMesh.globalCrackField(localPatchMaterials);
	  ...
	  forAll(mesh().boundaryMesh()[patchI], facei)
	    {
	      ...
	      if(ownerMat != neighbourMat)
                { 
		  result.boundaryField()[patchI][facei] = interfaceSigmaMax(ownerMat, neighbourMat);
                }
	      globalIndex++;
            }
        }
	...
	// philipc
	  ...
	  {
	    const scalarField ownerMatField = materials_.boundaryField()[patchI].patchInternalField();
	    const scalarField neighbourMatField = materials_.boundaryField()[patchI].patchNeighbourField();
...
Foam::tmp<Foam::surfaceScalarField> Foam::multiMaterialCohesiveLaw::tauMax() const
{
    tmp<surfaceScalarField> tresult
    (
        new surfaceScalarField
	  ...
	  forAll(mesh().boundaryMesh()[patchI], facei)
	    {
	      ...
	      if(ownerMat != neighbourMat)
		{ 
		  result.boundaryField()[patchI][facei] = interfaceTauMax(ownerMat, neighbourMat);
                }
	      globalIndex++;
            }
        }
...
Foam::tmp<Foam::surfaceScalarField> Foam::multiMaterialCohesiveLaw::GIc() const
{
    ...
    forAll(mesh().boundary(), patchI)
    {
	  ...
	  forAll(mesh().boundaryMesh()[patchI], facei)
	    {
	      ...
	      label neighbourMat = label(globalPatchMaterials[gcfa[globalIndex]]);
                ...
                if(ownerMat != neighbourMat)
                { 
		  result.boundaryField()[patchI][facei] = interfaceGIc(ownerMat, neighbourMat); 
                }
		globalIndex++;
            }     
        }
	  ...
	  {
	    const scalarField ownerMatField = materials_.boundaryField()[patchI].internalField();
	    const scalarField neighbourMatField = materials_.boundaryField()[patchI].patchNeighbourField();
...
Foam::tmp<Foam::surfaceScalarField> Foam::multiMaterialCohesiveLaw::GIIc() const
{
    ...
    forAll(mesh().boundary(), patchI)
    {
	  ...
	  forAll(mesh().boundaryMesh()[patchI], facei)
	    {
	      ...
	      if(ownerMat != neighbourMat)
                { 
		  result.boundaryField()[patchI][facei] = interfaceGIIc(ownerMat, neighbourMat); 
                } 
	      globalIndex++;
            }     
        }
	  ...
	  {	    
	    ...
	    forAll(mesh().boundaryMesh()[patchI], facei)
	      {
                // label ownerMat = materials_.boundaryField()[patchI][i];
                // label neighbourMat = materials_.boundaryField()[patchI][i+size];
                label ownerMat = label(ownerMatField[facei]);
                label neighbourMat = label(neighbourMatField[facei]);
...
Foam::scalar Foam::multiMaterialCohesiveLaw::interfaceSigmaMax(double mat1, double mat2) const
{
...
Foam::scalar Foam::multiMaterialCohesiveLaw::interfaceTauMax(double mat1, double mat2) const
{
  label interfaceLawID = interfaceID(mat1, mat2);
...
Foam::scalar Foam::multiMaterialCohesiveLaw::interfaceGIc(double mat1, double mat2) const
{
  label interfaceLawID = interfaceID(mat1, mat2);
...
Foam::scalar Foam::multiMaterialCohesiveLaw::interfaceGIIc(double mat1, double mat2) const
{
  label interfaceLawID = interfaceID(mat1, mat2);
 ...
 ) const
{
  ...
  if(ownerMat != neighbourMat)
    { 
      ...
      //Info << "face " << faceID  << " (*this)[matID] " << interfaceCohesiveLaws_[matID].name() << endl;
      interfaceCohesiveLaws_[matID].damageTractions(tN, tS, deltaN, deltaS, GI, GII, faceID, globalPatchMaterials);
    }
  else
    {
      ...
      //Info << "face " << faceID  << " (*this)[matID] " << (*this)[matID].name() << endl;
      (*this)[matID].damageTractions(tN, tS, deltaN, deltaS, GI, GII, faceID, globalPatchMaterials);
    }
}
 ...
 ) const
{
  tmp<surfaceVectorField> tresult
    ...
    //   + lambda1*tr(sGradU)*n;
    result =
    2*mu1*(n&fvc::interpolate(symm(gradU))) + lambda1*n*fvc::interpolate(tr(gradU));
    ...
    /*        forAll(result.internalField(), faceI)
	{
		...
		tensor ownSGradU = ((I-ownN*ownN)&gradU.internalField()[owner[faceI]]);
		tensor ngbSGradU = ((I-ngbN*ngbN)&gradU.internalField()[neighbour[faceI]]);
		...
		scalar ngbDn = (1.0/mesh().deltaCoeffs().internalField()[faceI]) - ownDn;
		...
		vector tractionN = 
			      ...
			      + ((own2ML*ngbDn*ngbLambda*ngbN*ngbTrSGradUt) + (ngb2ML*ownDn*ownLambda*ngbN*ownTrSGradUt))	
			      / ((own2ML*ngbDn) + (ngb2ML*ownDn));

		vector tractionT =
				faceMu*(ownUt - ngbUt)/(ownDn+ngbDn)
			      + ((ownMu*ngbMu*ngbDn*ngbSGradUn) + (ownMu*ngbMu*ownDn*ownSGradUn))
		...
		//vector ownDelta = mesh().C()[neighbour[faceI]] - mesh().C()[owner[faceI]];
		//vector ngbDelta = mesh().C()[owner[faceI]] - mesh().C()[neighbour[faceI]];
	    }
        }
    */
	// philipc
 ...
 //if(mesh().boundaryMesh()[patchI].type() == processorPolyPatch::typeName)
		...
		const scalarField neighbourMatField = materials_.boundaryField()[patchI].patchNeighbourField();
		const labelList& faceCells = mesh().boundary()[patchI].faceCells();
		const tensorField ngbGradUField = gradU.boundaryField()[patchI].patchNeighbourField();
		const scalarField ngbMuField = mu.boundaryField()[patchI].patchNeighbourField();
		const scalarField ngbLambdaField = lambda.boundaryField()[patchI].patchNeighbourField();
		const vectorField ngbUField = U.boundaryField()[patchI].patchNeighbourField();
		...
		forAll(mesh().boundaryMesh()[patchI], faceI)
		  {
			...
			//tensor ownSGradU = ((I-ownN*ownN)&gradU.boundaryField()[patchI][owner[faceI]]);
			tensor ownSGradU = ((I-ownN*ownN)&gradU.internalField()[faceCelli]);
			//tensor ngbSGradU = ((I-ngbN*ngbN)&gradU.boundaryField()[patchI][neighbour[faceI]]);
			...
			//scalar ngbLambda = lambda.boundaryField()[patchI][neighbour[faceI]];
			...
			//vector ngbUn = ngbN*(ngbN&U.boundaryField()[patchI][neighbour[faceI]]);
			...
			scalar ngbDn = (1.0/mesh().deltaCoeffs().boundaryField()[patchI][faceI]) - ownDn;
			
			scalar own2ML = (2*ownMu + ownLambda);
			scalar ngb2ML = (2*ngbMu + ngbLambda);
			
			// scalar face2ML = 1.0/((1.0-mesh().weights().boundaryField()[patchI][faceI])
			// 		      / own2ML + mesh().weights().boundaryField()[patchI][faceI]/ngb2ML);
			scalar face2ML = 1.0/((1.0-weights[faceI])
					      / own2ML + weights[faceI]/ngb2ML);
			// scalar faceMu = 1.0/((1.0-mesh().weights().boundaryField()[patchI][faceI])
		...
		//Pout << "max mag trac " << max(mag(result.boundaryField()[patchI])) << endl;
	      }
	    
	      else*/ if(mesh().boundary()[patchI].type() == cohesiveFvPatch::typeName)
	      {
		// I think mu and lambda are wrong on new crack faces so use internal cell values
		const scalarField muPatch = mu.boundaryField()[patchI].patchInternalField();
		const scalarField lambdaPatch = lambda.boundaryField()[patchI].patchInternalField();
		result.boundaryField()[patchI] =
		  (2*muPatch*n.boundaryField()[patchI]&symm(gradU.boundaryField()[patchI]))
		  + (lambdaPatch*tr(gradU.boundaryField()[patchI])*n.boundaryField()[patchI]);
	      }
