...
Foam::directTopoChange& Foam::repatch::meshMod()
{
...
Foam::repatch::repatch(polyMesh& mesh)
:
    mesh_(mesh),
    meshModPtr_(NULL)
{}
...
void Foam::repatch::changePatches
(
...
)
{
    if (meshModPtr_.valid())
    {
        FatalErrorIn
        (
            "repatch::changePatches(const List<polyPatch*>&)"
...
void Foam::repatch::changePatchID
(
    const label faceID,
    const label patchID
)
...
void Foam::repatch::changeAnchorPoint
(
    const label faceID,
    const label fp
)
