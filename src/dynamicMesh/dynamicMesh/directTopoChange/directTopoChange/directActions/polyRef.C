...
namespace Foam
{
    defineTypeNameAndDebug(polyRef, 0);
...
)
{
    ...
    forAll(elems, i)
    {
        ...
        if (newI >= len)
        {
            FatalErrorIn("polyRef::reorder(..)") << abort(FatalError);
        }
...
Foam::label Foam::polyRef::addFace
(
...
// for the face)
Foam::label Foam::polyRef::addInternalFace
(
    directTopoChange& meshMod,
...
Foam::scalar Foam::polyRef::getLevel0EdgeLength() const
{
    if (cellLevel_.size() != mesh_.nCells())
    {
        FatalErrorIn("scalar polyRef::getLevel0EdgeLength() const")
    ...
    if (level0Size == -1)
    {
        FatalErrorIn("polyRef::getLevel0EdgeLength()")
...
Foam::label Foam::polyRef::getAnchorCell
(
...
void Foam::polyRef::getFaceNeighbours
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const label faceI,
...
Foam::label Foam::polyRef::findMinLevel(const labelList& f) const
{
...
Foam::label Foam::polyRef::findMaxLevel(const labelList& f) const
{
...
Foam::label Foam::polyRef::countAnchors
(
    const labelList& f,
...
Foam::label Foam::polyRef::findLevel
(
    const face& f,
    const label startFp,
...
) const
{
    ...
    forAll(f, i)
    {
        ...
        {
            FatalErrorIn("polyRef::findLevel")
                << "face:" << f
...
void Foam::polyRef::checkInternalOrientation
(
    directTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
void Foam::polyRef::checkBoundaryOrientation
(
    directTopoChange& meshMod,
    const label cellI,
    const label faceI,
...
// splitPoint.
void Foam::polyRef::insertEdgeSplit
(
...
Foam::label Foam::polyRef::storeMidPointInfo
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
...
void Foam::polyRef::walkFaceToMid
(
    const labelList& edgeMidPoint,
    const label cLevel,
    const label faceI,
...
Foam::label Foam::polyRef::faceConsistentRefinement
(
    const bool maxSet,
...
void Foam::polyRef::setInstance(const fileName& inst)
{
    if (debug)
    {
        Pout<< "polyRef::setInstance(const fileName& inst) : "
...
Foam::polyRef::polyRef(const polyMesh& mesh)
:
    mesh_(mesh),
    cellLevel_
    (
    ...
    savedCellLevel_(0)
{
    if (history_.active() && history_.visibleCells().size() != mesh_.nCells())
    {
        FatalErrorIn
        (
            "polyRef::polyRef(const polyMesh&)"
...
Foam::labelList Foam::polyRef::cellPoints(const label cellI) const
{
    // Pick up points of the cell
    const cell& cFaces = mesh_.cells()[cellI];
...
) const
{
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "polyRef::consistentRefinement : Changed " << nChanged
...
) const
{
    ...
    if (maxFaceDiff <= 0)
    {
        FatalErrorIn
        (
            "polyRef::consistentSlowRefinement"
    ...
    while(true)
    {
        if (debug)
        {
            Pout<< "polyRef::consistentSlowRefinement : Seeded "
    ...
    if (debug)
    {
        Pout<< "polyRef::consistentSlowRefinement : From "
...
) const
{
    ...
    if (maxFaceDiff <= 0)
    {
        FatalErrorIn
        (
            "polyRef::consistentSlowRefinement2"
            "(const label, const labelList&, const labelList&)"
        )   << "Illegal maxFaceDiff " << maxFaceDiff << nl
            << "Value should be >= 1" << exit(FatalError);
    }
    ...
    while (true)
    {
        ...
        if (debug)
        {
            Pout<< "polyRef::consistentSlowRefinement2 : Changed " << nChanged
                << " refinement levels due to 2:1 conflicts."
                << endl;
        }
    ...
    if (debug)
    {
        Pout<< "polyRef::consistentSlowRefinement2 : From "
            << cellsToRefine.size() << " to " << newCellsToRefine.size()
            << " cells to refine." << endl;
...

        {
            ...
            Pout<< "polyRef::consistentSlowRefinement2 : writing "
...
Foam::labelListList Foam::polyRef::setRefinement
(
...
)
{
    ...
    if (debug)
    {
        ...
        Pout<< "polyRef::setRefinement : Dumping " << splitCells.size()
    ...
    if (debug)
    {
        ...
        Pout<< "polyRef::setRefinement : Dumping " << splitFaces.size()
    ...
    if (debug)
    {
        Pout<< "polyRef::setRefinement : Splitting faces" << endl;
    }
    ...
    if (debug)
    {
        ...
        {
            FatalErrorIn("polyRef::setRefinement(..)")
...
void Foam::polyRef::updateMesh(const mapPolyMesh& map)
{
...
)
{
...

    {
        ...
        {
            // Assume it is after polyRef that this routine is called.
        ...
        else
        {
            ...
            forAll(cellMap, newCellI)
            {
                ...
                if (oldCellI == -1)
                {
                    //FatalErrorIn("polyRef::updateMesh(const mapPolyMesh&)")
        ...
        //{
        //    WarningIn("polyRef::updateMesh(const mapPolyMesh&)")
        //        << "Problem : "
...
)
{
    ...
    if (history_.active())
    {
        WarningIn
        (
            "polyRef::subset(const labelList&, const labelList&"
            ", const labelList&)"
...
void Foam::polyRef::distribute(const mapDistributePolyMesh& map)
{
    if (debug)
    {
        Pout<< "polyRef::distribute :"
            << " Updating various lists"
            << endl;
    }
...
void Foam::polyRef::checkMesh() const
{
    ...
    if (debug)
    {
        Pout<< "polyRef::checkMesh : Using matching tolerance smallDim:"
            << smallDim << endl;
    }
    ...
    if (debug)
    {
        Pout<< "polyRef::checkMesh : Returning" << endl;
    }
}


void Foam::polyRef::checkRefinementLevels
(
    const label maxPointDiff,
    const labelList& pointsToCheck
) const
{
    if (debug)
    {
        Pout<< "polyRef::checkRefinementLevels :"
    ...
    {
        FatalErrorIn("polyRef::checkRefinementLevels(const label)")
...

    {
        ...
        for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++)
        {
            ...
            if (mag(cellLevel_[own] - cellLevel_[nei]) > 1)
            {
                FatalErrorIn
                (
                    "polyRef::checkRefinementLevels(const label)"
...
Foam::labelList Foam::polyRef::getSplitPoints() const
{
    if (debug)
    {
        checkRefinementLevels(-1, labelList(0));
    ...
    if (!history_.active())
    {
        FatalErrorIn("polyRef::getSplitPoints()")
...
//void Foam::polyRef::markCellClusters
//(
//    const label maxLevel,
//    labelList& cluster
//) const
...
) const
{
    if (debug)
    {
        Pout<< "polyRef::consistentUnrefinement :"
    ...
    if (maxSet)
    {
        FatalErrorIn
        (
            "polyRef::consistentUnrefinement(const labelList&, const bool"
...
void Foam::polyRef::setUnrefinement
(
    ...
    if (debug)
    {
        ...
        forAll(cellLevel_, cellI)
        {
            if (cellLevel_[cellI] < 0)
            {
                FatalErrorIn
                (
                    "polyRef::setUnrefinement(const labelList&, "
                    "polyTopoChange&)"
        ...
        Pout<< "polyRef::setRefinement : Dumping " << pSet.size()
            << " points and "
    ...
    forAll(splitPointLabels, i)
    {
        ...
        //if (debug)
        {
            ...
            forAll(pCells, j)
            {
                ...
                if (region == -1)
                {
                    FatalErrorIn("polyRef::setUnrefinement(..)")
                        << "Ininitial set of split points to unrefine does not"
