...
defineTypeNameAndDebug(polyRefinementHistory, 0);
...
void Foam::polyRefinementHistory::writeEntry
(
    const List<splitPolyCell>& splitCells,
    const splitPolyCell& split
)
{
    // Write me:
...
void Foam::polyRefinementHistory::writeDebug
(
...
//- Construct null
Foam::polyRefinementHistory::splitPolyCell::splitPolyCell()
:
    parent_(-1),
    addedCellsPtr_(NULL)
{}
...
Foam::polyRefinementHistory::splitPolyCell::splitPolyCell(const label parent)
:
    parent_(parent),
    addedCellsPtr_(NULL)
{}
...
Foam::polyRefinementHistory::splitPolyCell::splitPolyCell(Istream& is)
{
    is >> *this;
}
...
Foam::polyRefinementHistory::splitPolyCell::splitPolyCell(const splitPolyCell& sc)
:
    parent_(sc.parent_),
    addedCellsPtr_
    (
        sc.addedCellsPtr_.valid()
      ? new labelList(sc.addedCellsPtr_())
      : NULL
    )
{}
...
(
    Istream& is, polyRefinementHistory::splitPolyCell& sc
)
{
    ...
    {
        sc.addedCellsPtr_.reset(new labelList(addedCells));
    }
    else
    {
...
(
    Ostream& os,
    const polyRefinementHistory::splitPolyCell& sc
)
{
...
void Foam::polyRefinementHistory::checkIndices() const
{
    // Check indices.
    forAll(visibleCells_, i)
    {
        ...
        {
            FatalErrorIn("polyRefinementHistory::checkIndices() const")
...
Foam::label Foam::polyRefinementHistory::allocateSplitCell
(
    const label parent,
    const label i,
    const label nCells
)
{
    ...
    if (freeSplitCells_.size() > 0)
    {
        ...
        splitCells_[index] = splitPolyCell(parent);
    }
    else
    {
        index = splitCells_.size();

        splitCells_.append(splitPolyCell(parent));
    }
    ...
    if (parent >= 0)
    {
        splitPolyCell& parentSplit = splitCells_[parent];

        if (!parentSplit.addedCellsPtr_.valid())
        {
            // Allocate storage on parent for the new nCells cells
            parentSplit.addedCellsPtr_.reset(new labelList(nCells, -1));
        }


        // Store me on my parent
        labelList& parentSplits = parentSplit.addedCellsPtr_();
...
void Foam::polyRefinementHistory::freeSplitCell(const label index)
{
    splitPolyCell& split = splitCells_[index];

    // Make sure parent does not point to me anymore.
    if (split.parent_ >= 0)
    {
        autoPtr<labelList>& subCellsPtr =
            splitCells_[split.parent_].addedCellsPtr_;

        if (subCellsPtr.valid())
        {
            labelList& subCells = subCellsPtr();

            label myPos = findIndex(subCells, index);

            if (myPos == -1)
            {
                FatalErrorIn("polyRefinementHistory::freeSplitCell")
...
void Foam::polyRefinementHistory::markSplit
(
    const label index,
    labelList& oldToNew,
    DynamicList<splitPolyCell>& newSplitCells
) const
{
    if (oldToNew[index] == -1)
    {
        ...
        const splitPolyCell& split = splitCells_[index];
        ...
        if (split.addedCellsPtr_.valid())
        {
            const labelList& splits = split.addedCellsPtr_();
...
Foam::polyRefinementHistory::polyRefinementHistory(const IOobject& io)
:
    regIOobject(io)
{
    if
    ...
    if (debug)
    {
        Pout<< "polyRefinementHistory::polyRefinementHistory :"
...
//- Read or construct
Foam::polyRefinementHistory::polyRefinementHistory
(
    const IOobject& io,
    const List<splitPolyCell>& splitCells,
    const labelList& visibleCells
    ...
    freeSplitCells_(0)
{
    ...
    {
        splitCells_.setSize(nCells, splitPolyCell());
...
(
    const IOobject& io,
    const polyRefinementHistory& rh
)
:
    regIOobject(io),
    ...
    visibleCells_(rh.visibleCells())
{
    if (debug)
    {
        Pout<< "polyRefinementHistory::polyRefinementHistory : constructed initial"
            << " history." << endl;
    }
}
...
// Construct from Istream
Foam::polyRefinementHistory::polyRefinementHistory(const IOobject& io, Istream& is)
:
    regIOobject(io),
    splitCells_(is),
    freeSplitCells_(0),
...
void Foam::polyRefinementHistory::resize(const label size)
{
    label oldSize = visibleCells_.size();

    if (debug)
    {
        Pout<< "polyRefinementHistory::resize from " << oldSize << " to " << size
            << " cells" << endl;
    }
...
void Foam::polyRefinementHistory::updateMesh(const mapPolyMesh& map)
{
    if (active())
    {
        ...
        forAll(visibleCells_, cellI)
        {
            if (visibleCells_[cellI] != -1)
            {
                ...
                if (splitCells_[index].addedCellsPtr_.valid())
                {
                    FatalErrorIn
                    (
                        "polyRefinementHistory::updateMesh(const mapPolyMesh&)"
        ...
        if (debug)
        {
            Pout<< "polyRefinementHistory::updateMesh : from "
                << visibleCells_.size()
...
void Foam::polyRefinementHistory::subset
(
...
void Foam::polyRefinementHistory::countProc
(
    const label index,
...
void Foam::polyRefinementHistory::distribute(const mapDistributePolyMesh& map)
{
    if (!active())
    {
        FatalErrorIn
        (
            "polyRefinementHistory::distribute(const mapDistributePolyMesh&)"
    ...
    // If all n cells resulting from a single parent are sent across in one
    // go it will also send across that part of the refinement history.
    // If however e.g. first 1 and then the other n - 1 are sent across the
...
//Pout<< "polyRefinementHistory::distribute :"
//    << " destination:" << destination << endl;
    ...
    for (label procI = 0; procI < Pstream::nProcs(); procI++)
    {
        ...
        DynamicList<splitPolyCell> newSplitCells(splitCells_.size());
...
//                << endl;
        ...
        forAll(visibleCells_, cellI)
        {
            ...
            if (index >= 0 && destination[cellI] == procI)
            {
                ...
                oldToNew[index] = newSplitCells.size();
                newSplitCells.append(splitPolyCell(parent));
            }
        }
        ...
        {
            splitPolyCell& split = newSplitCells[index];
            ...
            if (split.addedCellsPtr_.valid())
            {
                labelList& splits = split.addedCellsPtr_();
    ...
    for (label procI = 0; procI < Pstream::nProcs(); procI++)
    {
        ...
        List<splitPolyCell> newSplitCells(fromNbr);
...
void Foam::polyRefinementHistory::compact()
{
    if (debug)
    {
        Pout<< "polyRefinementHistory::compact() Entering with:"
        ...
        forAll(freeSplitCells_, i)
        {
            ...
            {
                FatalErrorIn("polyRefinementHistory::compact()")
                    << "Problem index:" << index
                    << abort(FatalError);
            }
        }
    ...
    if (debug)
    {
        Pout<< "polyRefinementHistory::compact : compacted splitCells from "
    ...
    if (debug)
    {
        Pout<< "polyRefinementHistory::compact() NOW:"
            << " freeSplitCells_:" << freeSplitCells_.size()
            << " splitCells_:" << splitCells_.size()
...
void Foam::polyRefinementHistory::writeDebug() const
{
    writeDebug(visibleCells_, splitCells_);
}


void Foam::polyRefinementHistory::storeSplit
(
    const label cellI,
...
)
{
    ...
    else
    {
        ...
        parentIndex = allocateSplitCell(-1, -1, addedCells.size());
    }
    ...
    forAll(addedCells, i)
    {
        ...
        // are visible.
        visibleCells_[addedCellI] =
            allocateSplitCell(parentIndex, i, addedCells.size());

    }
}


void Foam::polyRefinementHistory::combineCells
(
...
)
{
    ...
    splitPolyCell& parentSplit = splitCells_[parentIndex];
    parentSplit.addedCellsPtr_.reset(NULL);
    visibleCells_[masterCellI] = parentIndex;
}


bool Foam::polyRefinementHistory::readData(Istream& is)
{
    is >> *this;
    return !is.bad();
}
...
bool Foam::polyRefinementHistory::writeData(Ostream& os) const
{
    os << *this;
...
Foam::Istream& Foam::operator>>(Istream& is, polyRefinementHistory& rh)
{
...
Foam::Ostream& Foam::operator<<(Ostream& os, const polyRefinementHistory& rh)
{
    const_cast<polyRefinementHistory&>(rh).compact();
