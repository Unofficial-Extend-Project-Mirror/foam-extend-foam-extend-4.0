...
template<>
const char*
Foam::NamedEnum<Foam::refineImmersedBoundaryMesh::ibCellCollection, 3>
::names[] =
{
    "undefined",
    "ibCells",
...
const Foam::NamedEnum<Foam::refineImmersedBoundaryMesh::ibCellCollection, 3>
...
) const
{
    ...
    {
        if (isA<immersedBoundaryPolyPatch>(mesh_.boundaryMesh()[patchI]))
        {
            Info<< "Found immersed boundary patch " << patchI
                << " named " << mesh_.boundaryMesh()[patchI].name()
                << endl;
            ...
            forAll (c, cI)
            {
                // No search required.  Duplicates are automatically removed
                // HJ, 1/Dec/2017
...
) const
{
    ...
    forAll (mesh_.boundaryMesh(), patchI)
    {
        if (isA<immersedBoundaryPolyPatch>(mesh_.boundaryMesh()[patchI]))
        {
            ...
            forAll (c, cellI)
            {
                // Get ibCell neighbours
                const labelList& curCells = cellCells[c[cellI]];
